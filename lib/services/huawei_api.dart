...
class HuaweiApi {
  ...
  Future<Map<String, String>> getWlanSsidNames() async {
    final response = await http.get(
      ...
      headers: {..._headers(), 'Accept': '*/*', 'Referer': '$baseUrl/index.asp'},
    );
    ...
    for (final match in matches) {
      ...
      if (band == '2.4GHz' || domain.endsWith('.WLANConfiguration.1')) result['2.4'] = ssid;
      if (band == '5GHz' || domain.endsWith('.WLANConfiguration.5')) result['5'] = ssid;
    }
    return result;
  }
  ...
  Future<HuaweiWifiSettings> get2GWifiSettings() async {
    final response = await http.get(
      ...
      headers: {..._headers(), 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8', 'Referer': '$baseUrl/index.asp'},
    );
    ...
    String selectValue(String name, {String fallback = ''}) {
      final escaped = RegExp.escape(name);
      final select = RegExp('<select[^>]+name=["\']$escaped["\'][^>]*>(.*?)</select>', caseSensitive: false, dotAll: true).firstMatch(body)?.group(1);
      if (select == null) return fallback;
      final selected = RegExp('<option[^>]+value=["\']([^"\']*)["\'][^>]*selected[^>]*>', caseSensitive: false).firstMatch(select);
  ...
  Future<void> update2GWifiSettings({required HuaweiWifiSettings settings, String? password}) async {
    if (_token == null || _cookie == null) throw Exception('Huawei session is not available. Please log in again.');
    ...
    final body = <String, String>{
      ...
      'w.SsidInst': '1', 'w.SSID': settings.ssid, 'w.Enable': settings.enabled ? '1' : '0',
      'w.Standard': '11bgn', 'w.BasicAuthenticationMode': 'None', 'w.BasicEncryptionModes': 'TKIPandAESEncryption',
      ...
      'w.SSIDAdvertisementEnabled': settings.broadcastSsid ? '1' : '0', 'w.WMMEnable': settings.wmmEnabled ? '1' : '0',
      'w.MaxAssociateNum': settings.maxAssociateNum.toString(), 'w.BeaconType': 'WPAand11i',
      'w.WEPEncryptionLevel': '104-bit', 'w.WEPKeyIndex': '1', 'x.X_HW_Token': _token!,
    };
    ...
    final response = await http.post(uri, headers: {..._headers(), 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8', 'Content-Type': 'application/x-www-form-urlencoded', 'Origin': baseUrl, 'Referer': '$baseUrl/html/amp/wlanbasic/WlanBasic.asp?2G'}, body: body);
    if (response.statusCode != 200) throw Exception('Unable to save 2.4 GHz Wi-Fi settings (HTTP ${response.statusCode}).');
  ...
  Future<HuaweiSimpleWifiSettings> getSimpleWifiSettings() async {
    ...
    if (response.statusCode != 200) throw Exception('Wi-Fi configuration request failed: HTTP ${response.statusCode}');
    final body = response.body;
    String value(String name, {String fallback = ''}) {
      final escaped = RegExp.escape(name);
      for (final pattern in [
    ...
    return HuaweiSimpleWifiSettings(
      band2G: HuaweiSimpleWifiBandSettings(enabled: checked('m.Enable', fallback: true), ssid: names['2.4'] ?? value('w0.SSID', fallback: 'HUAWEI-2.4G'), broadcastSsid: checked('w0.SSIDAdvertisementEnabled', fallback: true), wmmEnabled: checked('m.WMMEnable', fallback: true), staIsolation: checked('m.STAIsolation'), maxAssociateNum: intValue('m.MaxAssociateNum', 32)),
      band5G: HuaweiSimpleWifiBandSettings(enabled: checked('m.Enable5G', fallback: true), ssid: names['5'] ?? value('w1.SSID', fallback: 'HUAWEI-5G'), broadcastSsid: checked('w1.SSIDAdvertisementEnabled', fallback: true), wmmEnabled: checked('m.WMMEnable5G', fallback: true), staIsolation: checked('m.STAIsolation5G'), maxAssociateNum: intValue('m.MaxAssociateNum5G', 32)),
    );
  }
  ...
  Future<void> updateSimpleWifiSettings({required HuaweiSimpleWifiBandSettings band2G, required HuaweiSimpleWifiBandSettings band5G, String? password2G, String? password5G}) async {
    ...
    final uri = Uri.parse('$baseUrl/html/amp/wlanbasic/set.cgi').replace(queryParameters: query);
    final body = <String, String>{
      'w0.SSID': band2G.ssid, 'w0.SSIDAdvertisementEnabled': band2G.broadcastSsid ? '1' : '0',
      'm.SSID': band2G.ssid, 'm.SSIDAdvertisementEnabled': band2G.broadcastSsid ? '1' : '0', 'm.SsidInst': '1',
      'm.Enable': band2G.enabled ? '1' : '0', 'm.WMMEnable': band2G.wmmEnabled ? '1' : '0', 'm.STAIsolation': band2G.staIsolation ? '1' : '0', 'm.MaxAssociateNum': band2G.maxAssociateNum.toString(),
      'w1.SSID': band5G.ssid, 'w1.SSIDAdvertisementEnabled': band5G.broadcastSsid ? '1' : '0',
      'm.SSID5G': band5G.ssid, 'm.SSIDAdvertisementEnabled5G': band5G.broadcastSsid ? '1' : '0', 'm.SsidInst5G': '5',
      'm.Enable5G': band5G.enabled ? '1' : '0', 'm.WMMEnable5G': band5G.wmmEnabled ? '1' : '0', 'm.STAIsolation5G': band5G.staIsolation ? '1' : '0', 'm.MaxAssociateNum5G': band5G.maxAssociateNum.toString(),
      'x.X_HW_Token': _token!,
    };
    ...
    final response = await http.post(uri, headers: {..._headers(), 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8', 'Content-Type': 'application/x-www-form-urlencoded', 'Origin': baseUrl, 'Referer': '$baseUrl/html/amp/wlanbasic/simplewificfg.asp'}, body: body);
    if (response.statusCode != 200) throw Exception('Unable to save Wi-Fi settings (HTTP ${response.statusCode}).');
    final responseBody = response.body.toLowerCase();
    if (responseBody.contains('login.asp') && responseBody.contains('username')) throw Exception('Huawei session expired. Please log in again.');
  }
  ...
  Future<void> update5GWifiSettings({required HuaweiSimpleWifiBandSettings settings, String? password}) async {
    ...
    await updateSimpleWifiSettings(band2G: current.band2G, band5G: settings, password5G: password);
  }

  String _decodeHuaweiValue(String value) => value.replaceAll(r'\x3a', ':').replaceAll(r'\x5f', '_').replaceAll(r'\x2e', '.').replaceAll(r'\x2d', '-').replaceAll(r'\x5c', r'\').replaceAll(r'\"', '"');
